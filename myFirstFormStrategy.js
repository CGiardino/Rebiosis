...
module.exports = function(options) {
    ...
    function validate_credentials(executionScope, request, response, callback) {
        setTimeout(function() {
            ...
            if (request.body && request.body.user && request.body.password) {
				Mydb.query(("SELECT password FROM USER where username="+ "'" +request.body.user +"'"), function(returnValue){
					console.log(returnValue);
					if ( request.body.password == returnValue && returnValue!=undefined) {
    ...
    that.setupRoutes = function(server) {
        server.use('/', connect.router(function routes(app) {
            app.post('/auth/form_callback',
            function(request, response) {
                request.authenticate([that.name],
